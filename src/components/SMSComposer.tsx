...
type Props = {
  donor: Donor
  onClose: () => void
  onRegenerateLive?: (currentDraft: SMSDraft) => void
}

export default function SMSComposer({ donor, onClose, onRegenerateLive }: Props) {
  const [draft, setDraft] = useState<SMSDraft>(() =>
    cannedSMSFor(donor.id, donor.name, slotTimeForDonor(donor.id))
  )
    ...
    >
      ...
      <div className="relative flex w-full max-w-lg flex-col bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between gap-2 border-t border-rc-stone bg-rc-mist px-5 py-3">
          <button
            type="button"
            onClick={() => onRegenerateLive?.({ ...draft, text })}
            className="text-xs font-medium text-rc-teal hover:text-rc-red"
          >
            Regenerate live (BYOK) ⟳
          </button>
